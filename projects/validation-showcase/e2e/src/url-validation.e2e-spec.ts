import {
  browser,
  element,
  by, ElementFinder, ExpectedConditions
} from 'protractor';
...
describe('URL validation', () => {
  ...
  it('should match previous url validation screenshot', async (done) => {
    ...
    await browser.wait(ExpectedConditions.elementToBeClickable(element(by.id('url-validation-input'))));
    await SkyHostBrowser.setWindowBreakpoint('lg');
    await element(by.css('#url-validation-input')).sendKeys('foobar');
    await browser.executeScript('document.getElementById("url-validation-input").blur()');
